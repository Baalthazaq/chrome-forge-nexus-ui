...
export function ExperiencesSection({ sheet, updateSheet, isEditing }: Props) {
      ...
      <div className="space-y-2">
            ...
            {isEditing ? (
              <>
                <Input
                  ...
                  className="flex-1 bg-gray-800/50 border-gray-600 text-gray-100 text-sm"
                />
                <Input
                  type="number"
                  value={exp.value}
                  onChange={(e) => updateExperience(i, 'value', e.target.value)}
                  className="w-16 text-center bg-gray-800/50 border-gray-600 text-gray-100 text-sm"
                />
              ...
              <>
                <span className="flex-1 text-gray-200 text-sm">{exp.text || 'Unnamed experience'}</span>
                <span className="text-gray-100 font-bold text-sm">+{exp.value}</span>
              </>
            )}
          </div>
        ))}
        {experiences.length === 0 && (
          <div className="text-gray-500 text-sm text-center py-4">
            {isEditing ? 'No experiences yet. Add one above.' : 'No experiences yet.'}
          </div>
        )}
      </div>
    </Card>
