...
export const FeatureLibraryPicker = ({ open, onClose, onSelect }: Props) => {
  ...
  const loadFeatures = async () => {
    ...
    // Load from bestiary_features library
    const { data: libraryFeatures } = await supabase
      .from('bestiary_features')
      .select('*')
      .order('name');

    // Also extract unique features from existing creatures
    const { data: creatures } = await supabase
      .from('bestiary_creatures')
      .select('features, tier');

    const creatureFeatureMap = new Map<string, any>();
    (creatures || []).forEach(c => {
      const feats = typeof c.features === 'string' ? JSON.parse(c.features) : c.features || [];
      feats.forEach((f: any) => {
        if (f.name && !creatureFeatureMap.has(f.name)) {
          creatureFeatureMap.set(f.name, { ...f, tier: c.tier, source: 'creature' });
        }
      });
    });

    // Merge: library features take priority
    const libraryNames = new Set((libraryFeatures || []).map((f: any) => f.name));
    const merged = [
      ...(libraryFeatures || []).map((f: any) => ({ ...f, source: 'library' })),
      ...Array.from(creatureFeatureMap.values()).filter(f => !libraryNames.has(f.name)),
    ];
